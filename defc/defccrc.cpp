...
const ushort Crc16Table[256]= 	 //crc16校验表
{
...
uchar crc_sum8(const uchar* pData, int iLength){
    uchar aSum = 0;
...
ushort crc_sum16(const uchar* pData, int iLength){
    ushort aSum = 0;
    for (int i = 0; i < iLength; ++i) {
        aSum =  aSum + pData[ i ];
    }
...
uchar crc_8(const uchar* pData, int iLength){
    uchar result = 0;
    for (int i = 0; i < iLength; ++i) {
...
ushort crc_16(const uchar* pData, int iLengthh){
    ushort result = 0;
...
ushort crc_yaban16(const uchar* pData, int iLength)
{
    uint8 a;
    uint16 i,j;
    regs.val=0xffff;
...
ushort crc_modbus(const uchar *pData, int iLength)
{
    uchar hi = 0xFF;
    uchar lo = 0xFF;
    uchar index;
    for (int i = 0; i < iLength; ++i) {
        index = hi ^ pData[i];
        hi = lo ^ ModbusTableHi[index];
        lo = ModbusTableLo[index];
    }
